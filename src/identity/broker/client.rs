...
use crate::identity::manager::{BrokerWorkloadKey, CaClientTrait, CertRequest, CompositeId};
use crate::tls;
...
impl SpiffeBrokerClient {
    ...
    /// Mint an SVID for `req`. Split out from the [`CaClientTrait`] impl so
    /// tests can drive it with a bare [`CertRequest`], and so the trait impl
    /// only has to flatten the cache key.
    #[instrument(skip_all, fields(identity = %req.identity))]
    async fn fetch_certificate_for(
        &self,
        req: &CertRequest,
...
impl CaClientTrait for SpiffeBrokerClient {
    type Key = BrokerWorkloadKey;

    async fn fetch_certificate(
        &self,
        id: &CompositeId<BrokerWorkloadKey>,
    ) -> Result<tls::WorkloadCertificate, Error> {
        self.fetch_certificate_for(&id.to_cert_request()).await
    }
}
...
mod tests {
    ...
    async fn attestor_error_short_circuits() {
        ...
        let err = client
            .fetch_certificate_for(&CertRequest::new(identity()))
            .await
