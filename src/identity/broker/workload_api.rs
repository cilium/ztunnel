...
use crate::identity::workload_api_proto::spiffe::workloadapi::{X509svidRequest, X509svidResponse};
...
impl WorkloadApiClient {
    ...
    async fn subscribe(&self) -> Result<tonic::codec::Streaming<X509svidResponse>, Error> {
...
fn into_svids(resp: X509svidResponse) -> Vec<WorkloadSvid> {
    ...
    info!(
        count = total,
        "received SVID update from SPIFFE Workload API"
    );
    svids
}
