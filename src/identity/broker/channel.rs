        ...
        Box::pin(async move {
            ...
            match svid_source {
                ...
                Some(src) => {
                    let cfg = src
                        .client_config()
                        .map_err(|e| std::io::Error::other(format!("svid source: {e}")))?;
...
impl UdsGrpcChannel {
    ...
    fn build(socket_path: PathBuf, svid_source: Option<Arc<SvidSource>>) -> Result<Self, Error> {
        let connector = UdsConnector {
            path: Arc::new(socket_path),
            svid_source,
        };
        let client = hyper_util::client::legacy::Client::builder(crate::hyper_util::TokioExecutor)
            .http2_only(true)
